Vimee Core version *0.15.1* is now available from:

  <https://download.vimee.org/vimee-0.15.1/>
...
A number of changes to the way Vimee Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 Vim per kB (0.00001 VME). This is to prepare for dropping the minimum transaction fee to 0.1 Vim per kB (0.0001 VME) in the next release.
